...
Table~\ref{tab:schedule} summarizes the project timeline. Development was tracked using GitHub Issues and Milestones on the project repository.

\begin{table}[H]
\centering
\caption{Project schedule with key milestones.}
\label{tab:schedule}
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{Date} & \textbf{Milestone} \\
\midrule
03/11/2026 & Project description released \\
03/14--03/20 & Topic selection, literature review, dataset exploration \\
03/20--03/26 & Data download pipeline, initial InceptionV3 experiments \\
03/26/2026 & \textbf{Proposal submitted} (GitHub link posted to Canvas) \\
03/27--04/06 & Baseline CNN implementation and initial training runs \\
04/07--04/13 & Weight-Focus variant implementation and training \\
04/14--04/20 & Mixture of Experts variant implementation and training \\
04/21--04/27 & Comparative analysis, visualization, and presentation preparation \\
04/27/2026 & \textbf{Presentation slides posted to GitHub} \\
04/28--04/30 & \textbf{Oral presentations} \\
04/30--05/05 & Final report writing and polishing \\
05/05/2026 & \textbf{Final report due} \\
\bottomrule
\end{tabular}
\end{table}
...
Following the reference paper, we use the \textbf{L1 loss} (Mean Absolute Error) for training:
\begin{equation}
  \mathcal{L} = \frac{1}{N} \sum_{i=1}^{N} \frac{1}{5} \sum_{k=1}^{5} \left| \hat{y}_k^{(i)} - y_k^{(i)} \right|
\end{equation}
...
\begin{enumerate}
  ...
  \item \textbf{Mass conditioning overhead}: In the Weight-Focus variant, errors in the mass prediction propagate to all other nutrient predictions, potentially amplifying mistakes rather than improving accuracy. The mass head's errors become a source of noise for the conditioned heads.

  \item \textbf{Regularization balance}: The larger FC layers in the Baseline model, combined with aggressive dropout ($p = 0.6$), seem to strike a better balance between capacity and regularization than the MoE's smaller experts.
\end{enumerate}
